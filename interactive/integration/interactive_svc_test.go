...
import (
	...
	"webooktrial/interactive/integration/startup"
...
func (s *InteractiveTestSuite) TestIncrReadCnt() {
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			tc.before(t)
			resp, err := s.server.IncrReadCnt(context.Background(),
				&intrv1.IncrReadCntRequest{
					Biz:   tc.biz,
					BizId: tc.bizId,
				})
...
func (s *InteractiveTestSuite) TestGet() {
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			tc.before(t)
			resp, err := s.server.Get(context.Background(), &intrv1.GetRequest{
				Biz:   tc.biz,
				BizId: tc.bizId,
				Uid:   tc.uid,
			})
...
func (s *InteractiveTestSuite) TestGetByIds() {
	...
	testCases := []struct {
		...
		},
		{
			...
			biz:     "test",
			ids:     []int64{100, 200},
			wantRes: &intrv1.GetByIdsResponse{Intrs: map[int64]*intrv1.Interactive{}},
		},
	}

	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			resp, err := s.server.GetByIds(context.Background(), &intrv1.GetByIdsRequest{
				Biz: tc.biz,
				Ids: tc.ids,
			})
			assert.Equal(t, tc.wantErr, err)
